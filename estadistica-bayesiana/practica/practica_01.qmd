...
imgs_dir <- here::here("estadistica-bayesiana", "practica", "imgs")
...
1.  ¿Es verdad que existen los vampiros? Versión Crepúsculo
    ...
    if (is_html) knitr::include_graphics(file.path(imgs_dir, "crepusculo.jpg"))
    ```
...
1.  Árboles enfermos
    ...
    * Compare las probabilidades a priori y a posteriori de que el árbol tenga moho. 
    ...
    if (is_html) knitr::include_graphics(file.path(imgs_dir, "ceibo.png"))
    ```
...
1.  Transporte El Impuntual
    ...
    * El colectivo de Franco no está demorado. 
    ...
    if (is_html) knitr::include_graphics(file.path(imgs_dir, "colectivo.jpg"))
    ```
...
1.  Bebé panda
    ...
    * Un tiempo después sos encontramos con que en la segunda parición da a luz a un único
    ...
    if (is_html) knitr::include_graphics(file.path(imgs_dir, "bebe_panda.jpg"))
    ```
