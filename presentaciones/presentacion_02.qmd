...
Para el caso binomial que venimos estudiando, consideramos una realización más. ¿Cuál es la probabilidad de obtener $A$ (agua)?

$$
\begin{align*}
P(\tilde{y} = 1\mid y) & = \int_0^1 \pi \vphantom{\tilde{y}}p(\theta\mid x) d \pi = \mathbb{E}[{p(\theta\mid x)}] \\
& = \frac{y+a}{y+a+N-x+b} = \frac{y+a}{N+a+b} \\
\end{align*}
$$
