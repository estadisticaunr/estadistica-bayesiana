...
Un posible _score_ predictivo para un determinado valor $y_i$ es la probabilidad que el modelo le asocia, 
$$\int p\left(y_i\mid\theta\right) p(\theta\mid y) d\theta \approx \frac{1}{S}\sum_{s=1}^S p(y_i\mid \theta^{(s)})$$ 

El _score_ predictivo total (para todas las posibles observaciones) es la _<span style="color:#1f7a8c;">log</span>-[posterior]{.underline} <span style="color:#EB8A90;">pointwise</span> <span style="color:#683257;">predictive density</span>_
