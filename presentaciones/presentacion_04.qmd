...
En la estadística bayesiana, la distribución _a posteriori_ es la base
de todas inferencia: combina el conocimiento _a priori_ con la
...
El riesgo _a posteriori_ (_posterior risk_ o _posterior expected loss_) es la
pérdida esperada ponderada por los valores de $\theta$ (y su distribución
_a posteriori_).
...
p <- tibble(x=seq(0,1,0.008),y=dbeta(x,2,9)) %>%
  ggplot() + 
  geom_line(aes(x=x,y=y),size=1,col="#FF729F") +
...
p1 <- ggplot() +
  ...
  geom_line(data=dv,aes(x=x,y=y),size=1,col="#FF729F") +
  geom_segment(aes(x=p.l,xend=p.h,y=0,yend=0),size=1.2) +
  ylab(expression("p("*theta*"|y)")) +
  xlab(expression(theta))
...
p2 <- ggplot() +
  ...
  geom_segment(aes(x=hpdi.lower,xend=hpdi.higher,y=0,yend=0),size=1.2) +
  ylab(expression("p("*theta*"|y)")) +
  xlab(expression(theta))
...
muestras_theta <- rbeta(2000,6,4)
mean(muestras_theta > 0.5)
mean(muestras_theta > 0.8)
```
...
¿Cuál es la distribución _a posteriori_ de la chance de obtener cara $\frac{\theta}{1-\theta}$? 
...
muestras_theta <- rbeta(2000,6,4)
muestras_odds <- muestras_theta/(1-muestras_theta)
```
...
muestras_theta <- rbeta(2000,6,4)
y_new <- rbinom(2000,11,muestras_theta)
```
