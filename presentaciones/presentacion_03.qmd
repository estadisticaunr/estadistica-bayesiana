...
dv <- tibble(x = seq(-3,6,length.out = 500),
       prior = dnorm(x,mu_prior,sigma_prior),
       likelihood = dnorm(x,y_bar,sigma_likelihood),
       posterior_posterior = dnorm(x,mu_prior*(sigma^2/(n*sigma_prior^2+sigma^2)) + y_bar * (n*sigma_prior^2/(n*sigma_prior^2+sigma^2)),
...
$$
p(\lambda \mid r,s) = p(\lambda) = \frac{r^s}{\Gamma(s)} x^{s-1}e^{-rx}
$$
...
$$p(\lambda \mid \mathbf{y}) \propto \frac{\lambda^{\sum_i y_i}e^{-n\lambda}}{\prod_{i}y_i!} \frac{r^s}{\Gamma(s)} \lambda^{s-1}e^{-r\lambda}$$ \pause
...
Por lo tanto, resulta que la distribución _a posteriori_ es Gamma de parámetros $n+r$ y $\sum_i y_i + s$
...
\begin{align*}
    ...
    \mu & \sim \mathrm{Gamma}(\theta,\tau^2)
\end{align*}
$$

El _likelihood_ es normal:
$$p(y_i\mid \mu) = \frac{1}{\sqrt{2\pi\sigma}} e^{-\frac{(y_i-\mu)^2}{2\sigma^2}} \rightarrow \\ p(\mathbf{y}\mid \mu) = \left(\frac{1}{2\pi\sigma}\right)^{n/2}  e^{-\frac{\sum_i(y_i-\mu)^2}{2\sigma^2}} \propto e^{-\frac{(\bar{y}-\mu)^2}{2\sigma^2/n}}$$
...
$$p(\mu\mid \mathbf{y}) \propto p(\mathbf{y}\mid\mu) p(\mu)$$ \pause
...
Representa un balance (promedio ponderado o \textit{combinación convexa}) entre la media muestral y la media esperada \textit{a priori}.
...
¿Cómo obtenemos una observación nueva si sabemos que $\mu = 2$ (sabiendo que $\sigma = 1,2$)? . . . Directamente tomamos una muestra $\tilde{y}$ de $\mathcal{N}\left(\mu=2,\sigma^2= 1.2^2\right)$  
...
p3<-ggplot() +
  geom_vline(xintercept=2, col = "#FF729F", size=1) +
  xlim(c(2-6,2+6)) +
  xlab(expression(mu))


p4 <- ggplot() +
  geom_line(data=tibble(x,y),aes(x=x,y=y),col="#FF729F",size=1) +
