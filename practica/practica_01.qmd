...
1.  **Paraguas**
    ...
    Si LLL es "los tres amigos dijeron que llovía", buscamos p(lluvia|LLL) =       p(LLL|lluvia)p(lluvia)/p(LLL).

    p(LLL|lluvia) es la probabilidad de que ninguno de los tres mienta.

    p(LLL) = p(LLL|lluvia) p(lluvia) + p(LLL|no lluvia) p(no lluvia).
...
1.  **Sherlock**
    ...
    p(E|S') es la probabilidad de observar sangre 0 y sangre AB, si Guido no       estaba en la escena del crimen. Esto es la probabilidad de que la primera      persona tuviera sangre 0 y la segunda AB más la probabilidad de que la         primera persona tuviera sangre AB y la segunda sangre 0: (60/100)x(1/100) +     (1/100)x(60/100) = 0.012.
...
1.  **¿Alguien ordena las medias?**
    ...
    P: elegir un par de medias iguales
    
    B: elegir un par de medias blancas 
    
    Matemáticamente, buscamos p(B|P) = p(B,C|P) + p(B,C'|P) (básicamente           estamos marginalizando la variable "cajón"). El segundo término es 0 porque     B y C' no pueden darse nunca (nunca voy a sacar un par blanco del cajón        RVA). Entonces queda p(B|P) = p(B,C|P) = p(B|C,P)p(C|P), que son los dos       valores que encontramos arriba.

    ¿De dónde sale que p(B,C|P) = p(B|C,P)p(C|P)?
    p(B,C|P) = p(B,C,P)/p(P) y el numerador, por regla de la cadena, es            p(P)p(C|P)p(B|C,P).
...
1.  **La Falacia del Fiscal**
    ...
    P(I|E) es solo similar a P(E|I) si P(I) es similar a P(E). Este no es el       caso, ¿por qué?
...
1.  **Con amigos así, quién necesita enemigos**
    ...
    Estamos tratando de hacer inferencias sobre T, es decir, la distribución a     posteriori debe ser p(T=1), p(T=2), ... p(T=6).
    
    Conviene analizar este problema en forma secuencial. Observamos "G", ¿qué      significa eso? ¿cómo obtenemos la verosimilitud? p(G|T=1) = 5/6, p(G|T=2) =     4/6, p(G|T=3) = 3/6...
...
1.  💻 **Orden en la sala**
    ...
    Si X=2, p(Y=2|X=2) = dbinom(2, size = 2, prob = 1/365)
    
    Si X=100, p(Y=2|X=100) = dbinom(2, size = 100, prob = 1/365)
...
1.  💻 **Puede fallar...**
    ...
    i. ¿Cuál es la distribución _a posteriori_ de $\theta$, la probabilidad de obtener cara?
...
1.  **¡Saludos a los cubos con puntos! (...) Serán dados**
    ...
    Estamos tratando de hacer inferencias sobre D1 y D2, los valroes de cada       uno de los datos. Definir la distribución a priori no es complicado (es una     distribución sobre D1 y D2).
    
    El truco está en determinar la verosimilitud de cada par (D1,D2) luego de      haber observado un 9.
    
    ¿Cuál es la probabilidad de observar 9 en la suma de los dos dado que ...?
    
    D1 = 1 y D2 = 1 - la probabilidad de observar 9 es 0
    
    D1 = 1 y D2 = 2 - la probabilidad de observar 9 es 0
    
    D1 = 4 y D2 = 5 - la probabilidad de observar 9 es 1
    
    D1 = 5 y D2 = 4 - la probabilidad de observar 9 es 1
