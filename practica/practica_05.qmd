...
1.  Curvas de crecimiento de tiranosáuridos
    ...
    En el panel izquierdo de @fig-dinos se puede observar que hay una relación no lineal
    ...
    El modelo que resulta para $\log(Y_{ij})$ es
    ...
    con $\mathbb{E}(Y_{ij}) = f_j(X_{ij})$ y $\sigma^2_j$ determina la varianza del error
    para la especie $j$.
    ...
    **Modelo 1**
    ...
    Observando el panel derecho de @fig-dinos se puede concluir que luego de transformar 
    ...
    i. Escriba un programa en Stan para ajustar este modelo. 
    ...
    **Modelo 2**
    ...
    considerando la naturaleza jerárquica del modelo.
    ...
    **Modelo 3**
    ...
    expresando los parámetros en función de parámetros no restringidos.
    ...
    * $d_j = \exp (\alpha_{j4})$
    ...
    i. Analizar resultados, convergencia, graficar, etc. etc.

    **Modelo 4**

    Explicar mejor

    Es el mismo modelo que el modelo 3, excepto que las especies tienen la misma varianza
    $\sigma^2_j = \sigma^2$ y los coeficientes de regresión se modelan de manera jerárquica.
    ...
    i. **Nota para nosotros:** Queremos usar los priors del libro? O mejor usamos otra cosa?
    Estoy mirando los InvGamma por ejemplo. 
    Ídem con la notación de varianza-desvío estándar.
