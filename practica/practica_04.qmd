...
1.  **Mejorando mi regresión bayesiana**
    ...
    Considere siguiente versión del modelo del ejercicio anterior que propone 
    distribuciones _a priori_ para los parámetros del modelo.
    $$
    \begin{aligned}
    \text{ventas}_i &= \beta_0 + \beta_1 \text{publicidad}_i + \varepsilon_i \\
    \varepsilon_i &\underset{iid}{\sim} \text{Normal}(0, \sigma^2)
    \beta_0 &\sim \text{Normal}(\bar{ventas_i}, 10) \\
...
1.  **La altura... ¿se hereda?**
    ...
    pares de madres e hijas registradas en un estudio realizado por Karl Pearson y 
    Alice Lee en 1903.
...
1.  **Clima en Australia**
    ...
    El conjunto de datos **...** contiene 100 días de datos climáticos para dos ciudades 
    de Australia: Uluru y Wollongong. Se intentará predecir la temperatura a las 3
...
1.  **Pingüinos**
    ...
    Considere el dataset de pingüinos de Palmer y los siguientes modelos:
...
1.  **Un _prior_ informativo marca la diferencia**
    ...
    Considere el conjunto de datos sobre belleza y proporción de sexos (`sexratio.csv`) .
    ...
    representa el grupo de belleza.
    ...
    ¿Qué puede concluir? 
    i. Proponga distribuciones _a prior_ informativas.
...
1.  **Vamos de pesca**
    ...
    * Retomar el ejercicio de los pescados. 
    * Proponer modelos que incrementan en complejidad
    * Hacer comparacion/seleccion de modelos
    * Un poco el chiste es que todos los predictores ayudan a predecir muy bien la respuesta
    pero al correlacion entre los predictores es alta, entonces no tiene sentido meter 
    muchos predictores.
    * En este modelo tambien meteriamos transformaciones en predictores y respuesta.
