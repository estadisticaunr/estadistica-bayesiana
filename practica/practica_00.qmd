...
# author: "Ing. Ignacio Evangelista"
# subauthor: "Colaboradores: Mg. Fernanda Méndez -- Dr. Ignacio Álvarez Castro"
# fecha: "2021"
# materia: "Introducción a la Estadística Bayesiana"
# header-includes:
...
1.  Sea $X$ una variable aleatoria con soporte $X \in \mathcal{S} = [1, \infty)$. 
    ...
    $f(x) = c \exp(-x / \theta)$ sea una función de densidad de probabilidad válida.
    <!-- @Reich2020 -->

1.  Suponga $X \sim \text{Uniforme}(a, b)$. Su soporte es $\mathcal{S} = [a, b]$ 
...
1.  Según personas expertas en un problema determinado, se indica que el valor de un parámetro 
...
1.  Las luces de giro en los automóviles se utilizan para indicar que se va a realizar
    ...
    Aunque menos frecuente, también se da que el vehículo colca la luz de giro, pero no gira. 
    La probabilidad de girar dado que se colca la luz de giro es 0.87 y la probabilidad de 
    girar dado que no se coloca la luz de giro es 0.21. Si observa que un vehículo coloca la 
    luz de giro, ¿cuál es la probabilidad de que efectivamente doble?
...
1.  Problema de los dos sobres
    ...
    dinero que el otro, pero a simple vista son indistinguibles. Se te pide que escogas
