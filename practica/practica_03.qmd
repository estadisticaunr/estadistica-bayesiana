...
1.  ¿Cuánto vale $\pi$?
    ...
    ggplot(df) + 
        ...
        geom_path(aes(x = x, y = y), linewidth = 1.5, color = "#0984C0") +
        geom_rect(
            xmin = -1, xmax = 1, ymin = -1, ymax = 1, 
            fill = NA, color = "#999999", linewidth = 1.5
        )
    ```
...
1. **Aproximación de grilla**
    ...
    i. Obtenga la distribución _a posteriori_ de $\theta$ y grafíquela.
    i. Obtenga la distribución _a posteriori_ de $\theta$ utilizando el método de la 
    grilla en base a una grilla de 10 puntos e incluya el resultado en el gráfico creado 
    anteriormente.
    ...
    utilizar una grilla más densa? Describa ventajas y desventajas.

1. **Cálculo de probabilidades en base a la grilla**

    En base al _posterior_ aproximado del ejercicio anterior calcule las siguientes
    probabilidades
...
1.  **Aproximación de grilla en 2 dimensiones**
    ...
    y sea el siguiente modelo de regresión
    ...
    mediante el método de la grilla. 
    i. Obtenga el _posterior_ marginal de $\alpha$
    i. Obtenga el _posterior_ marginal de $\beta$
    ...
    El gráfico de dispersión luce de la siguiente manera
...
1.  **Escalando la aproximación mediante grilla**

    **WIP**

    ¿Cuántos puntos son necesarios para estimar un _posterior_ por la aproximación de la 
    grilla? Suponga que se usa una grilla de 500 puntos en cada dimensión.
...
1.  **_Benchmark_**
    ...
    * Moraleja: El numero de puntos, el consumo de memoria, y el tiempo computacional
...
1.  Una compañía pesquera de Comodoro Rivadavia se encuentra probando un nuevo método para
    ...
    distribución predictiva a posteriori. Interprete los resultados.
