...
1.  **¿Quién dijo que el fútbol siempre da revancha?**
    ...
    4 a 2. Conside que el número de goles que un equipo hace en un partido puede modelizarse
...
1.  **La regla del tres**
    ...
    tesina antes de entregarla. Si en 20 páginas encontrase 5 _typos_,
    sería _razonable_ estimar que la probabilidad de que una página
    ...
    siendo $N$ el número de páginas. Para ello, grafique la distribución _a priori_
...
=======
    i. Derive el _posterior_ $p(\lambda \mid \mathbf{X})$ suponiendo que el _prior_ sobre 
    $\lambda$ es $\text{Gamma}(\alpha, \beta)$ con 
    $p(\lambda \mid \alpha, \beta) =  \propto \lambda^{\alpha - 1}e ^ {-\lambda \beta}$.  
>>>>>>> f89d9b8554dd668eced72a33a87737ba5041049d
...
1.  **Distribución predictiva a posteriori (a mano)**
    ...
    Considere un modelo $y\mid \theta \sim \mathrm{Bin}(\theta,n)$, donde $\theta$ puede tomar valores discretos $0,\ 0.1,\ 0.2,\ \dots,\ 1$. Se realizaron inferencias sobre $\theta$ y se obtuvo la distribución _a posteriori_ que se muestra en la parte superior de la @fig-ppd.

    Se desea obtener la distribución predictiva _a posteriori_ para el número de éxitos $\tilde{y}$ en $n=5$ nuevas realizaciones del experimento. Cada valor de $\theta$ da lugar a una posible distribución de $\tilde{y}$ de acuerdo a la verosimilitud binomial, como se observa en la parte inferior de la @fig-ppd.
