...
1.  **Otras distribuciones conjugadas (II)**
    ...
    \end{array}
    $$
    donde la función de densidad de la distribución binomial negativa es $f(y\mid \theta, m) = {y+m-1 \choose y} \theta^{m} (1-\theta)^y.
...
1.  **Otras distribuciones conjugadas (III)**
    ...
    \end{array}
    $$
    donde la función de densidad exponencial es $f(y\mid\theta) = \theta e^{-\lambda y}$
    
    Elija una distribución _a priori_ conjugada de la verosimilitud propuesta y obtenga la expresión para la distribución de probabilidad _a posteriori_
...
1.  **Entrada en calor**
    ...
    i. Intervalo del 95\% para $\pi$ siendo $\pi\mid y \sim \text{Beta}(4,5)$
    i. Intervalo del 60\% para $\pi$ siendo $\pi\mid y \sim \text{Beta}(4,5)$
    i. Intervalo del 89\% para $\lambda$ siendo $\lambda\mid y \sim \text{Gamma}(1,8)$
    i. Intervalo del 95\% para $\lambda$ siendo $\lambda\mid y \sim \text{Gamma}(2,5)$
    i. Intervalo del 81\% para $\mu$ siendo $\mu\mid y \sim \mathcal{N}(10,2^2)$
    i. Intervalo del 99\% para $\pi$ siendo $\mu\mid y \sim \mathcal{N}(-3,1^2)$
...
1.  Dada la distribución _a posteriori_ $p(\theta \mid y)$, probar que el estimador de Bayes
    que minimiza la pérdida $L_1$ es la mediana de $p(\theta \mid y)$
